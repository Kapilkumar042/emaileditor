...
const removeAllActiveBadge = () => {
  getShadowRoot()
    .querySelectorAll('.vikasit-email-merge-tag')
    .forEach((item) => {
      item.classList.remove('vikasit-email-merge-tag-focus');
    });

  const popoverNode = getShadowRoot().querySelectorAll(
    '.vikasit-email-merge-tag-popover'
  );
  if (popoverNode) {
  }
};
  ...
  useEffect(() => {
    ...
    const onClick: EventListenerOrEventListenerObject = (e) => {
      ...
      if (
        ...
        target.classList.contains('vikasit-email-merge-tag')
      ) {
        target.classList.add('vikasit-email-merge-tag-focus');
  ...
  return (
    <>
      ...
      {textContainer && createPortal(
        <div ref={popoverRef} onClick={onClick} className={classnames('vikasit-email-merge-tag-popover')}>
          <div className='vikasit-email-merge-tag-popover-container'>
            <h3>
              <span>{t('Default value')}</span>
              <IconFont style={{ color: 'rgb(92, 95, 98)' }} iconName='icon-close' onClick={onClose} />
            </h3>
            <div className={'vikasit-email-merge-tag-popover-desc'}>
              <p>
                {t('If a personalized text value isn\"t available, then a default value is shown.')}
              </p>
              <div className='vikasit-email-merge-tag-popover-desc-label'>
                <input autoFocus value={text} onChange={onChange} type="text" autoComplete='off' maxLength={40} />
                <div className='vikasit-email-merge-tag-popover-desc-label-count'>
                  {text.length}/40
                </div>
              </div>
              <div className='vikasit-email-merge-tag-popover-desc-label-button'>
