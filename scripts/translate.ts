...
const overwriteData = fs.readJsonSync(
  path.join(process.cwd(), 'packages/vikasit-email-localization/locales/overwrite.json'),
);
...
async function main() {
  ...
  Object.keys(translateWords).forEach(lan => {
    ...
    fs.writeFileSync(
      `packages/vikasit-email-localization/locales/${lan}.json`,
      JSON.stringify(combineData, null, 2),
    );
  });

  fs.writeFileSync(
    `packages/vikasit-email-localization/locales/locales.json`,
